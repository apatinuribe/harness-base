#!/usr/bin/env bash
# test_guard.sh — Pruebas del guard de scripts/hooks.sh (PreToolUse Edit|Write).
#
...
# caso <descripcion> <esperado> <dir de trabajo> <json>
caso() {
  local desc="$1" esperado="$2" dir="$3" json="$4"
  total=$((total + 1))
  local err
  err=$(cd "$dir" && printf '%s' "$json" | bash "$GUARD" guard 2>&1 >/dev/null)
  local rc=$?
